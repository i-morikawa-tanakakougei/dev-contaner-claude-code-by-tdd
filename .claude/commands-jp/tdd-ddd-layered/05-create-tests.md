Given-When-Then仕様とドメインモデルに基づいてTDDテストを作成する
...
> 🗺️ **現在位置**: スプリント実行フェーズ - TDDテスト作成（05/16）  
> 🎯 **フェーズ目的**: Given-When-Thenシナリオから失敗テストを作成（RED）  
> ⬅️ **前のステージ**: 04-domain-modeling（ドメインモデル設計）  
> ➡️ **次のステージ**: 06-implement-domain（ドメイン層実装）
>
> **📋 3層アーキテクチャ操作**:  
> - 🎯 **戦略的**: `docs/use_cases/core/index.md`（コアシナリオの参照）  
> - 📊 **戦術的**: `docs/use_cases/index.md`（TDD REDフェーズステータス更新）  
> - 🔧 **実行**: `docs/use_cases/issue-X-Y.json`（テスト作成詳細の追跡）

## 🔴 **TDD REDフェーズ: 失敗テストのみ**

**⚠️ 重要な注意:**
- **このステップはTDD REDフェーズ** - 仕様に基づいて失敗テストを作成
- **失敗テストのみ作成** - プロダクションコードは一切実装しない  
- **TDD規律** - テストは最初に失敗してTDDサイクルを検証
- **実装は禁止** - プロダクションコードは次のステップで作成

**TDDサイクル位置:**
1. `04-domain-modeling` ← 設計文書化
2. `05-create-tests` ← **【現在位置】TDD RED（失敗テスト）**
3. `06-implement-domain` ← TDD GREEN（テストを通す）
4. `11-refactor` ← TDD REFACTOR（コード品質改善）

**基本的なTDDルール:**
- ✅ 仕様を捉えた失敗テストを書く
- ❌ 実装コードは一切書かない
- ✅ 全テストが失敗することを確認（REDフェーズ確認）

## 📋 **TDD REDフェーズタスクチェックリスト**

**包括的な失敗テスト作成にこのチェックリストを使用してください:**

### 🔴 必須タスク

#### **📖 仕様分析**
- [ ] **ユースケース仕様を読む**: issue-X-Y.mdからGiven-When-Thenシナリオを解析
- [ ] **ドメインモデル設計を読む**: 設計文書からエンティティと振る舞いの期待値を抽出
- [ ] **シナリオをテストケースにマッピング**: 各Given-When-Thenを具体的なテストメソッドに変換
- [ ] **受け入れ基準を抽出**: 受け入れ基準をテスト可能なアサーションに変換

#### **🔴 失敗テスト実装**
- [ ] **ドメインエンティティテストを作成**: エンティティの作成、振る舞い、不変条件をテスト（失敗必須）
- [ ] **値オブジェクトテストを作成**: 不変性、バリデーション、等価性をテスト（失敗必須）
- [ ] **ドメインサービステストを作成**: ビジネスロジックとルール適用をテスト（失敗必須）
- [ ] **アプリケーションサービステストを作成**: ユースケースオーケストレーションをテスト（失敗必須）

#### **🔍 REDフェーズ検証**
- [ ] **全テストを実行**: pytestを使用して完全なテストスイートを実行
- [ ] **全テストの失敗を確認**: 実装不足によりすべてのテストが失敗することを確認
- [ ] **メタデータを更新**: issue-X-Y.jsonでTDD REDフェーズ完了をマーク
- [ ] **失敗テストをコミット**: 明確なコミットメッセージですべてのテストコードをバージョン管理

### 🟡 推奨タスク

#### **🏗️ テスト構造設計**
- [ ] **ユニットテストを設計**: 個別のエンティティ、値オブジェクト、ドメインサービスのテストを計画
- [ ] **統合テストを設計**: リポジトリインターフェースとアプリケーションサービスのテストを計画
- [ ] **E2Eテストを設計**: プレゼンテーション層を通した完全なユーザーシナリオのテストを計画
- [ ] **テスト構成を計画**: src/ディレクトリ階層に従ってテストファイルを構造化
- [ ] **テスト命名を設計**: ビジネス意図を捉えた説明的な名前を使用

#### **🧪 テストインフラ設定**
- [ ] **テストフィクスチャを作成**: テストデータとオブジェクトファクトリを設計
- [ ] **モックオブジェクトを作成**: 外部依存とリポジトリのモックを設計
- [ ] **テスト設定をセットアップ**: pytest設定と必要に応じてテストデータベースを設定
- [ ] **テストユーティリティを作成**: 共通テスト操作のヘルパー関数を構築

### 🟢 オプションタスク

#### **📋 高度なテストカバレッジ**
- [ ] **リポジトリインターフェーステストを作成**: データアクセス契約をテスト（失敗必須）
- [ ] **プレゼンテーションテストを作成**: APIエンドポイントと入力バリデーションをテスト（失敗必須）
- [ ] **テスト境界を特定**: ユニット vs 統合 vs E2Eテストの配置を決定
- [ ] **テスト分離を設定**: テストが独立して繰り返し可能であることを保証

#### **📊 フェーズ完了とハンドオフ**
- [ ] **テストカバレッジをチェック**: すべてのGiven-When-Thenシナリオに対応するテストがあることを確認
- [ ] **テスト品質を検証**: テストが読みやすく、保守可能で、焦点が絞られていることを確認
- [ ] **テスト実装を文書化**: テスト計画文書を作成
- [ ] **テスト実行を文書化**: テスト実行結果と失敗理由を記録
- [ ] **GREENフェーズの準備**: テストが実装すべき内容を明確に定義
- [ ] **プロダクションコードなしを確認**: 実装コードが一切書かれていないことを検証

**💡 プロTip**: この段階ではすべてのテストが失敗する必要があります - テストが通る場合は誤って何かを実装しています！  
...
4. **仕様とドメインモデルの分析**:
   ```bash
   # 📖 テストシナリオのための仕様とドメインモデル分析
   echo "📖 仕様とドメインモデル分析中..."
   
   # 仕様からGiven-When-Thenシナリオを抽出
   all_scenarios=()
   all_entities=()
...
6. **ユニットテストの作成（TDD REDフェーズ）**:
   ```bash
   # 🧪 TDD REDフェーズで包括的ユニットテストを作成
   ...
   # 値オブジェクトテストを作成
   value_objects=("EntityId" "Email")  # 共通値オブジェクト
...
8. **テストがRED状態であることを検証**:
   ...
   # RED状態を検証するクイックテストを実行
   red_verification_result=""
   if command -v pytest >/dev/null 2>&1; then
       echo "  ⚡ RED状態検証実行中..."
       
       # テストを実行し結果をキャプチャ（失敗するはず）
...
12. **最終成功とTDDガイダンス**:
    ...
    if commit_transaction; then
        ...
        echo ""
        echo "📋 作成されたテスト:"
        printf '   - %s\n' "${created_test_files[@]:0:5}"  # 最初の5個を表示
...
重要な注意事項:
- テストは最初に失敗する必要があります（TDD REDフェーズ）
- 各テストは一つのことのみをテストします
- snake_caseで説明的なテスト名を使用します
- テストにGiven-When-Thenコメントを含めます
- 外部依存を適切にモックします
