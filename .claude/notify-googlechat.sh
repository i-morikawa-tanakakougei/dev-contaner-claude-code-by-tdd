...
# Google Chat notification script for Claude Code hooks
# This script sends notifications to Google Chat when Claude Code events occur

# Debug: Log to file for troubleshooting
DEBUG_LOG="/workspace/.claude/notify-googlechat.log"
...
# Get Google Chat Webhook URL (from .env.dev or environment variable)
GOOGLE_CHAT_WEBHOOK_URL="${GOOGLE_CHAT_WEBHOOK_URL}"

# Check if webhook URL is set
if [ -z "$GOOGLE_CHAT_WEBHOOK_URL" ]; then
    echo "Error: GOOGLE_CHAT_WEBHOOK_URL is not set in $ENV_FILE or environment variables" >> "$DEBUG_LOG"
    exit 1
fi
...
echo "Sending to Google Chat: $NOTIFICATION_TYPE" >> "$DEBUG_LOG"
echo "Message: $MESSAGE" >> "$DEBUG_LOG"

# Create Google Chat payload (using text field as shown in the sample)
