Use the 08-implement-infra subagent to implement infrastructure layer for persistence and external services. This command MUST USE PROACTIVELY the specialized 08-implement-infra subagent for optimal infrastructure implementation.
...
As a fallback mechanism in case the specialized subagent 08-implement-infra is not properly invoked:
- MUST READ: /workspace/.claude/agents/08-implement-infra.md
...
## Task Execution
[Execute 08-implement-infra specific tasks]
...
3. **Agent Result Verification**:
   ...
   if ! verify_critical_tasks "08-implement-infra" "$latest_report"; then
       echo "⚠️ Critical tasks確認で問題が検出されました - 再実行を試行します"
       prepare_retry_context "08-implement-infra" "1" "${verification_issues[@]}"
       
       # Enhanced context for retry
       echo "🔄 再実行用の強化コンテキスト準備中..."
       prepare_enhanced_context "08-implement-infra" "$context_file" "${verification_issues[@]}"
