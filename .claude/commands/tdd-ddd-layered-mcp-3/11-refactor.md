# /refactor-enhanced [issue-numbers] - MCP-3統合インテリジェントリファクタリング

## 🎯 Purpose and Scope

Execute TDD REFACTOR phase with MCP-3 enhanced intelligence for code quality improvement and technical debt reduction. This MCP-3 enhanced command provides systematic analysis, automated code analysis, pattern recognition, and intelligent refactoring recommendations.
...
- **Systematic Refactoring Analysis**: Sequential MCPによる体系的リファクタリング分析と段階的品質改善
...
4. **Systematic Refactoring Strategy**: Sequential MCPによる体系的リファクタリング戦略と段階的改善
5. **Intelligent Analysis**: Leverage Serena for deep code quality analysis
6. **Pattern-Driven Improvement**: Apply Context7 refactoring patterns and industry best practices
7. **Automated Optimization**: Use MCP-3 intelligence for systematic code improvement
...
- **Systematic Analysis Coverage**: 95%の体系的リファクタリング分析適用
...
- Sequential MCPによる体系的リファクタリング戦略分析と段階的改善計画
...
Execute this comprehensive setup to initialize MCP-3 enhanced refactoring environment:
...
# MCP-3 availability check
echo "🚀 MCP-3 Enhanced Refactoring with Intelligent MCP Integration..."

# Step 1: 状況分析・複雑性判断（文脈駆動）
echo "📊 Analyzing task complexity and requirements..."
echo "🧪 Refactoring task - Code quality improvement with systematic analysis"
...
if [[ "$MCP_SEQUENTIAL" == "available" ]]; then
    echo "🧩 Complex refactoring scenario detected - Using Sequential MCP for systematic analysis"
    echo "✅ Task meets Sequential MCP criteria: Systematic refactoring analysis needed"
    MCP3_REFACTOR_MODE="ENHANCED"
else
    echo "⚡ Standard refactoring task - Using Serena + Context7 integration"
    MCP3_REFACTOR_MODE="STANDARD"
fi
...
# Initialize MCP-3 enhanced refactoring environment
ISSUE_NUMBERS="$1"
echo "🎯 MCP-3 Enhanced Refactoring Target: Issues [$ISSUE_NUMBERS]"
if [[ "$MCP3_REFACTOR_MODE" == "ENHANCED" ]]; then
    echo "✅ MCP-3 Enhanced Refactoring Mode Activated"
    echo "🔍 Available Capabilities:"
    echo "  • Sequential: Systematic refactoring strategy and quality improvement"
    echo "  • Serena: Code quality analysis and technical debt discovery"
    echo "  • Context7: Refactoring patterns and best practices integration"
    echo "  • Comprehensive: Multi-layer refactoring analysis and optimization"
else
    echo "📋 Standard refactoring mode with available MCP tools"
fi
...
if [[ "$MCP_CONTEXT7" == "unavailable" ]]; then
    echo "📚 Running without latest refactoring patterns - using standard approaches"
fi

if [[ "$MCP_SEQUENTIAL" == "unavailable" && "$MCP3_REFACTOR_MODE" == "ENHANCED" ]]; then
    echo "🧩 Complex refactoring task but Sequential MCP unavailable"
    echo "📋 Using structured manual approach with step-by-step refactoring checklists"
fi
...
3. **Sequential MCP Systematic Refactoring Analysis**

- Use mcp__sequential-thinking__sequentialthinking to systematically analyze: "Comprehensive refactoring strategy for issues ${ISSUE_NUMBERS}: Need to analyze code quality patterns, identify technical debt, prioritize improvement opportunities, create step-by-step refactoring plan with quality validation."

4. **Automated Code Quality Discovery**
...
5. **Intelligent Technical Debt Mining**
...
**MCP-3 Enhanced Design (if available):**
