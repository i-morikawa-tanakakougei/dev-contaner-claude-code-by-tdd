...
if [[ -n "$ISSUE_NUMBER" ]]; then
    echo "Retrieving GitHub issue #$ISSUE_NUMBER with comments for emergency recovery..."
    ...
    echo "Prioritizing latest 5 comments for emergency recovery analysis"
    
    # Check for emergency context in comments
    if [[ $COMMENT_COUNT -gt 0 ]]; then
        echo "Analyzing comment timeline for emergency context..."
        # Recent comments take precedence for emergency understanding
        LATEST_COMMENT_DATE=$(echo "$RECENT_COMMENTS" | jq -r '.[0].createdAt // empty')
        if [[ -n "$LATEST_COMMENT_DATE" ]]; then
            echo "Latest emergency update: $LATEST_COMMENT_DATE"
        fi
        
        # Extract emergency-related comments
        echo "Extracting emergency-related context..."
        echo "$RECENT_COMMENTS" | jq -r '.[] | select(.body | contains("emergency") or contains("urgent") or contains("critical") or contains("production") or contains("hotfix")) | .body' | head -3
    fi
fi
```
